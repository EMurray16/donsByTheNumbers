...
// NewRSEXP creates an RSEXP object from the function input. It attempts to create an RSEXP object from any input, and
...
// to a C.SEXP), but checking the "coercibility" of an input is impossible at compile time in the rsexp package. This
// is why the function tries to coerce the input into a C.SEXP.
//
...
//
// NewRSEXP uses a combination of reflection and the unsafe package to coerce the input into rsexp's C.SEXP type.
...
// NotASEXP error.
//
// The rsexp package does not support all of R's SEXP types. If the input provided is a C.SEXP but not a supported
// type, then the TypeMismatch error is returned.
func NewRSEXP(in any) (r RSEXP, err error) {
	...
	typeEnum := C.TYPEOF(r)
	// Even if we have a C.SEXP, we still have no guarantee that the SEXP is of a type that is compatible with our
	// rsexp package
...
// AsNumeric extracts data from the input RSEXP and returns it as a slice. The result is a slice that contains the
// same data as the R vector represented by the RSEXP. If the underlying data cannot be coerced into numeric data, the
// TypeMismatch error is returned.
func AsNumeric[t RNumeric](r RSEXP) (out []t, err error) {
	//start by finding the length of the SEXP and making a slice
	Slen := int(C.XLENGTH(r))
	out = make([]t, Slen)

	rsexpType := C.TYPEOF(r)
...
// AsCharacter extracts the data from th einptu RSEXP and returns it as a slice. The result is a slice that contains
// the same data as the R vector represented by the RSEXP. If the underlying data connot be coerced into string data,
// the TypeMismatch error is returned.
...
// will have the same data and dimensions as the input Matrix and be of the REALSXP type (aka a double in R). Because
// the intent of this function is to prepare data to be sent back to R, which largely treats doubles and integers the
// same, this function cannot return an RSEXP of type INTSXP.
...
// CharacterToRSEXP converts a slice of string (or byte slice) data into a C.SEXP, represented by the returned RSEXP
// data. The R representation will have the same data as the inptu slice and be the STRSXP type (aka the character type
// in R).
...
// as row indexes, starting at 1 instead of 0 to be consistent with R.
//
...
//
// MakeDataFrame also checks that the type of all the provided data columns are valid types according to rsexp, and
// that they can be used to create a column in a data frame, or a TypeMismatch error will be returned. Right now, this
// list includes integer vectors, real vectors, and string vectors only. Examples of invalid types include lists, data
// frames, or other nested objects.
...
// MakeList creates an R list from the provided inputs and returns its representing RSEXP object.
...
// match, a LengthMismatch error is returned.
...
// Export converts an input RSEXP object into the caller's provided C.SEXP type. It is used as a final function to
// prepare data to be sent back to R. The input type is any, because the rsexp package cannot anticipate the strict
// C.SEXP type used by the caller.
//
// Like NewRSEXP, however, Export checks the type parameter provided using reflection and returns a NotASEXP
// error if it is not a C.SEXP.
func Export[t any](r *RSEXP) (out t, err error) {
