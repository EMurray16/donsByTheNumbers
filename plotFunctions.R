...
masterColorList = function() {
	return(
		c("fivethirtyeight"="#ed713a", "Wimbledon"="#0000ff", # 538 points plot
			...
			"Cumulative"=rgb(0.7,0.6,0.8), # gdae plots
			"Offense" = rgb(0.35,0.7,0.9), "Defense" = rgb(0.9,0.6,0), "Combined" = "black", #luck plots
			"Possession" = rgb(0.35,0.6,0.5), "Shot Share" = rgb(0.35,0.7,0.9), "Avg Possession" = "darkgreen", "Avg Shot Share" = rgb(0,0.45,0.7)
		))
}
...
make538Plots <- function(mergeTable) {
	...
	g1 = ggplot() + baseTheme() +
		geom_line(data=mergeTable, aes(x=date, y=cumProjPoints, color="fivethirtyeight"), size=1) +
		geom_line(data=mergeTable, aes(x=date, y=adjProjPoints), color="grey", size=0.5) +
	...
	if (spiPercentIncrease > 0) {
		spiCaption = paste("The Dons' SPI has **increased ", spiPercentIncrease, "%** since the start of the season, from ",
											 mergeTable$wimbledonSPI[1], " to ", mergeTable$wimbledonSPI[nrow(mergeTable)], ".", sep="")
	} else if (spiPercentIncrease < 0) {
		spiCaption = paste("The Dons' SPI has **decreased ", spiPercentIncrease, "%** since the start of the season, from ",
											 mergeTable$wimbledonSPI[1], " to ", mergeTable$wimbledonSPI[nrow(mergeTable)], ".", sep="")
	} else {
		spiCaption = paste("The Dons' SPI is the same as it was at the start of the season - ", mergeTable$wimbledonSPI[1], ".", sep="")
	}
	
	g2 = ggplot() + baseTheme() +
		geom_line(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=wimbledonSPI), color="#0000ff") +
	...
	g3 = ggplot() + baseTheme() +
		geom_bar(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=importance), fill="black", stat="identity") +
...
makeXGPlots <- function(mergeTable) {
	...
	g1 = ggplot(xgPlotTable) + baseTheme() +
		...
		geom_bar(data=xgPlotTable, aes(x=date, y=gFor, fill="Goals For"), stat="identity") +
		geom_bar(data=xgPlotTable, aes(x=date, y=0-gOpp, fill="Goals Against"), stat="identity") +
		geom_line(data=xgPlotTable, aes(x=date, y=xgFor, color="xG For"),size=1) +
		geom_line(data=xgPlotTable, aes(x=date, y=0-xgOpp, color="xG Against"), size=1) +
		geom_point(data=xgPlotTable, aes(x=date, y=xgFor, color="xG For"), size=2) +
		geom_point(data=xgPlotTable, aes(x=date, y=0-xgOpp, color="xG Against"), size=2) +
	...
	g2 = ggplot(xgPlotTable) + baseTheme() +
		geom_line(data=xgPlotTable, aes(x=date, y=GDAE), color=rgb(0.8,0.6,0.7)) +
		geom_point(data=xgPlotTable, aes(x=date, y=GDAE), color=rgb(0.8,0.6,0.7)) +
		geom_line(data=xgPlotTable, aes(x=date, y=cumGDAE, color="Cumulative")) +
		geom_point(data=xgPlotTable, aes(x=date, y=cumGDAE, color="Cumulative")) +
		geom_hline(yintercept=0) + 
		labs(x=NULL, y="GDAE") +
	...
	g3 = ggplot(luckPlotTable) + baseTheme() +
		...
		geom_line(data=luckPlotTable, aes(x=date, y=luckOff, color="Offense")) + 
		geom_line(data=luckPlotTable, aes(x=date, y=luckDef, color="Defense")) +
		geom_line(data=luckPlotTable, aes(x=date, y=luckComb, color="Combined")) +
		geom_point(data=luckPlotTable, aes(x=date, y=luckOff, color="Offense")) + 
		geom_point(data=luckPlotTable, aes(x=date, y=luckDef, color="Defense")) +
		geom_point(data=luckPlotTable, aes(x=date, y=luckComb, color="Combined")) +
		labs(x=NULL, y="Luck") +
		scale_color_manual(values=masterColorList()) +
		facet_grid(~Model) +
...
makeBasicStatPlots <- function(mergeTable) {
	...
	g1 = ggplot() + baseTheme() +
		...
		geom_line(data=mergeTable[hasHappened == TRUE], aes(x=date, y=cumPossess, color="Avg Possession"), size=1.25) +
		geom_point(data=mergeTable[hasHappened == TRUE], aes(x=date, y=cumPossess, color="Avg Possession"), size=1.5) + 
		geom_line(data=mergeTable[hasHappened == TRUE], aes(x=date, y=cumShotShare, color="Avg Shot Share"), size=1.25) +
		geom_point(data=mergeTable[hasHappened == TRUE], aes(x=date, y=cumShotShare, color="Avg Shot Share"), size=1.5) + 
	...
	g2 = ggplot(mergeTable) + baseTheme() + 
		geom_line(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=cumGFor, color="Goals For")) +
		geom_point(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=cumGFor, color="Goals For")) +
		geom_line(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=cumGOpp, color="Goals Against")) +
		geom_point(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=cumGOpp, color="Goals Against")) +
		labs(x=NULL, y="Goals") + 
		scale_color_manual(values=masterColorList()) +
		ggtitle("Season Total Goals For and Against") + ylim(0,max(c(mergeTable$cumGFor, mergeTable$cumGOpp)))
	
	g3 = ggplot(mergeTable) + baseTheme() + 
		geom_bar(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=gFor, fill="Goals For"), stat="identity") +
		geom_bar(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=0-gOpp, fill="Goals Against"), stat="identity") +
		geom_hline(yintercept=0) +
		geom_line(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=goalDiff, color="Net Goal Difference"), size=2) +
		geom_point(data=mergeTable[hasHappened == TRUE,], aes(x=date, y=goalDiff, color="Net Goal Difference"), size=3) +
		labs(x=NULL, y="Goals") + 
